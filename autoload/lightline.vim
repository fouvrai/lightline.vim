...
let s:_lightline = {
      ...
      \   },
      \   'component_type': {
      \     'tabs': 'tabsel', 'buffers': 'tabsel', 'close': 'raw'
      \   },
      \   'tab_component': {},
      ...
      \   },
      \   'buffer_component': {},
      \   'buffer_component_function': {
      \     'filename': 'lightline#buffer#filename', 'modified': 'lightline#buffer#modified',
      \     'readonly': 'lightline#buffer#readonly', 'bufnum': 'lightline#buffer#bufnum'
      \   },
      \   'colorscheme': 'default',
      \   'mode_map': {
...
function! lightline#init() abort
  ...
  for f in values(s:lightline.buffer_component_function)
    silent! call eval(f, [1])
  endfor
  let s:mode = ''
endfunction
...
function! s:tabline(nr, range, onefn) abort
  let [x, y, z] = [[], [], []]
  let nr = a:nr
  let cnt = a:range[-1]
  for i in a:range
    call add(i < nr ? x : i == nr ? y : z, '%'. i . 'T%{lightline#' . a:onefn . '(' . i . ',' . (i == nr) . ')}' . (i == cnt ? '%T' : ''))
  endfor
  let abbr = '...'
...
function! lightline#tabs() abort
  return s:tabline(tabpagenr(), range(1, tabpagenr('$')), 'onetab')
endfunction
...
function! lightline#buffers() abort
  ...
  let nr = bufnr('%')
  let bufnrs = filter(range(1, bufnr('$')), nr . ' == v:val || buflisted(v:val)')
  return s:tabline(nr, bufnrs, 'onebuffer')
endfunction

function! s:one(n, active, compmap, comps, compfns) abort
  let _ = []
  let tab_component = a:comps
  let tab_component_function = a:compfns
  for name in a:active ? a:compmap.active : a:compmap.inactive
    if has_key(tab_component_function, name)
      call add(_, call(tab_component_function[name], [a:n]))
    else
      call add(_, get(tab_component, name, ''))
    endif
  endfor
